...
ltoman.columns
# %%
ltcameroon.columns.symmetric_difference(ltoman.columns)
ltall = pd.concat([ltcameroon, ltoman])
# %%
ltall.country.value_counts()
# %%
ltall[["country", "station", "temperature", "latitude", "latabs"]].sample(
    5, random_state=3
)
# %%
...
ltall[["country", "station", "month", "temperature", "latitude"]].sample(
    5, random_state=1
)
# %%
...
ltall.groupby(["country"]).agg(
    {"temperature": ["mean", "max", "count"], "latabs": ["mean", "max", "count"]}
)
# %%
