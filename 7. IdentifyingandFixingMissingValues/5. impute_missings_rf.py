...
nls97["parentincome"] = nls97.parentincome.replace(list(range(-5, 0)), np.nan)

# load the wage income and associated data
wagedatalist = ["wageincome20", "weeksworked20", "parentincome", "hdegnum"]
wagedata = nls97.loc[nls97.weeksworked20 > 0, wagedatalist]


# use miss forest to impute values
imputer = MissForest()
wagedataimp = imputer.fit_transform(wagedata)
wagedatalistimp = ["wageincomeimp", "weeksworked20imp", "parentincomeimp"]
wagedataimp.rename(
    columns={
...
# %%
wagedata[["wageincome20", "wageincome20imp", "weeksworked20", "weeksworked20imp"]].agg(
    ["count", "mean", "std"]
)
...
kernel = mf.ImputationKernel(
    data=wagedata[wagedatalist], save_all_iterations=True, random_state=1
)
kernel.mice(3, verbose=True)

wagedataimpmice = kernel.complete_data()

wagedataimpmice.rename(
    columns={
...
wagedata = wagedata[wagedatalist].join(
    wagedataimpmice[["wageincome20impmice", "weeksworked20impmice"]]
)

# %%
wagedata[
    ["wageincome20", "wageincome20impmice", "weeksworked20", "weeksworked20impmice"]
].agg(["count", "mean", "std"])
