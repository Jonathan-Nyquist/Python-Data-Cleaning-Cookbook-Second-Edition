...
nls97weeksworked["missingwageincome"] = np.where(
    nls97weeksworked.wageincome20.isnull(), 1, 0
)
# %%
nls97weeksworked.groupby(["missingwageincome"])[
...
nls97weeksworked.parentincome.fillna(nls97weeksworked.parentincome.mean(), inplace=True)
nls97weeksworked["degltcol"] = np.where(nls97weeksworked.hdegnum <= 2, 1, 0)
nls97weeksworked["degcol"] = np.where(nls97weeksworked.hdegnum.between(3, 4), 1, 0)
nls97weeksworked["degadv"] = np.where(nls97weeksworked.hdegnum > 4, 1, 0)
...
def getlm(df, ycolname, xcolnames):
    ...
    coefficients = pd.DataFrame(
        zip(["constant"] + xcolnames, lm.params, lm.pvalues),
...
pred = (
    lm.predict(sm.add_constant(nls97weeksworked[xvars]))
    .to_frame()
    .rename(columns={0: "pred"})
)
nls97weeksworked = nls97weeksworked.join(pred)

nls97weeksworked["wageincomeimp"] = np.where(
    nls97weeksworked.wageincome20.isnull(),
    nls97weeksworked.pred,
    nls97weeksworked.wageincome20,
)
pd.options.display.float_format = "{:,.0f}".format
# %%
nls97weeksworked[["wageincomeimp", "wageincome20"] + xvars].sample(10, random_state=7)
# %%
nls97weeksworked[["wageincomeimp", "wageincome20"]].agg(["count", "mean", "std"])

# add an error term
np.random.seed(0)
randomadd = np.random.normal(0, lm.resid.std(), nls97weeksworked.shape[0])
randomadddf = pd.DataFrame(
    randomadd, columns=["randomadd"], index=nls97weeksworked.index
)
nls97weeksworked = nls97weeksworked.join(randomadddf)
nls97weeksworked["stochasticpred"] = nls97weeksworked.pred + nls97weeksworked.randomadd

nls97weeksworked["wageincomeimpstoc"] = np.where(
    nls97weeksworked.wageincome20.isnull(),
