...
nls97["age2"] = nls97.apply(lambda x: relativedelta(rundate, x.birthdate).years, axis=1)
...
firstcase = (
    covidcases.loc[covidcases.new_cases > 0, ["location", "casedate"]]
    .sort_values(["location", "casedate"])
    .drop_duplicates(["location"], keep="first")
    .rename(columns={"casedate": "firstcasedate"})
)
covidcases = pd.merge(
    covidcases, firstcase, left_on=["location"], right_on=["location"], how="left"
)
