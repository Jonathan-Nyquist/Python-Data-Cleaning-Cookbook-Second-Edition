...
landtemps["elevation_group"] = np.where(
    landtemps.elevation > landtemps.elevation.quantile(0.8), "High", "Low"
)
landtemps.elevation_group = landtemps.elevation_group.astype("category")
# %%
landtemps.groupby(["elevation_group"], observed=False)["elevation"].agg(
    ["count", "min", "max"]
)

# use the numpy where function to create a categorical series with 3 values
landtemps["elevation_group"] = np.where(
    landtemps.elevation > landtemps.elevation.quantile(0.8),
    "High",
...
nls97.loc[[999406, 750699], "colenrfeb00":"colenroct04"].T
nls97["baenrollment"] = (
    nls97.filter(like="colenr").transform(lambda x: x.str[0:1] == "3").any(axis=1)
)
