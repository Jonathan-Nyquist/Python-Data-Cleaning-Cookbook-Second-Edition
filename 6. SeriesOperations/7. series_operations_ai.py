...
# %%
nls97sdf[["childnum", "childathome", "childnotathome"]].sample(5, random_state=1)

nls97sdf = nls97sdf.chat(
    "evermarried is 'No' when maritalstatus is 'Never-married', else 'Yes'"
)
# %%
...
firstcase = covidcases.sort_values(["location", "casedate"]).drop_duplicates(
    ["location"], keep="first"
)
...
firstcase[["iso_code", "continent", "casedate", "total_cases", "new_cases"]].head(2).T

# try it with a pandasai smart data frame
covidcasessdf = SmartDataframe(covidcases, config={"llm": llm})

firstcasesdf = covidcasessdf.chat(
    "Show first casedate and location and other values for each country."
)
# %%
firstcasesdf.shape
# firstcasesdf = firstcasesdf.chat("Make location the index.")
firstcasesdf[["location", "continent", "casedate", "total_cases", "new_cases"]].head(
    2
).T
