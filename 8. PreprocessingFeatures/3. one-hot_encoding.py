...
X_demo_train, X_demo_test, y_demo_train, y_demo_test = train_test_split(
    nls97demo[feature_cols], nls97demo[["wageincome20"]], test_size=0.3, random_state=0
)

# use get dummies to create dummies features
pd.get_dummies(X_demo_train, columns=["gender", "maritalstatus"], dtype=float).head(2).T
# %%
pd.get_dummies(
    X_demo_train, columns=["gender", "maritalstatus"], dtype=float, drop_first=True
).head(2).T
...
# %%
X_demo_train.colenroct99.sort_values().unique()
# %%
X_demo_train.head()
# %%
X_demo_train.info()

oe = OrdinalEncoder(categories=[X_demo_train.colenroct99.sort_values().unique()])
