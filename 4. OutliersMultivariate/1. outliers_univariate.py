...
thirdq, firstq = covidtotalsonly.total_cases.quantile(
    0.75
), covidtotalsonly.total_cases.quantile(0.25)
...
def getoutliers():
    ...
    for col in covidtotalsonly.columns[1:]:
        thirdq, firstq = covidtotalsonly[col].quantile(0.75), covidtotalsonly[
            col
        ].quantile(0.25)
        interquartilerange = 1.5 * (thirdq - firstq)
        ...
        df = covidtotals.loc[
            (covidtotals[col] > outlierhigh) | (covidtotals[col] < outlierlow)
        ]
