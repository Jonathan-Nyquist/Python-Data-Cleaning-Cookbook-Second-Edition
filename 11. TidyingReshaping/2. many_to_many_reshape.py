...
cma.drop_duplicates(["itemid", "citation"]).itemid.count()
cma.drop_duplicates(["itemid", "creatorid"]).itemid.count()
# %%

# show a collection item with duplicated citations and creators
cma.set_index(["itemid"], inplace=True)
# %%
cma.loc[124733, ["title", "citation", "creation_date", "creator", "birth_year"]].head(6)

# create a collections data frame
collectionsvars = ["title", "category", "creation_date"]
cmacollections = (
    cma[collectionsvars].reset_index().drop_duplicates(["itemid"]).set_index(["itemid"])
)
# %%
...
cmacreators["birth_year"] = (
    cmacreators.birth_year.str.findall("\d+").str[0].astype(float)
)

youngartists = cmacreators.loc[cmacreators.birth_year > 1950, ["creator"]].assign(
    creatorbornafter1950="Y"
)
youngartists.shape[0] == youngartists.index.nunique()
# %%
youngartists

cmacollections = pd.merge(
    cmacollections, youngartists, left_on=["itemid"], right_on=["itemid"], how="left"
)
