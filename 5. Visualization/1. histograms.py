...
# %%
landtemps[["station", "country", "latabs", "elevation", "avgtemp"]].sample(
    10, random_state=1
)
...
plt.hist(
    [getcases(k) for k in showregions],
...
for j, ax in enumerate(axes):
    ax.hist(
        covidtotals.loc[covidtotals.region == showregions[j]].total_cases_pm, bins=7
    )
